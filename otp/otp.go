...
func isNumberOfDigitsValid(val int) error {
	if val < minNumOfDigits {
		return fmt.Errorf("The OTP struct is not valid: Number of digits used (%d) is less than the minimum (%d)", val, minNumOfDigits)
	}
	return nil
}
...
func (totp Totp) isDataValid() error {
	if !validInterval(totp.Interval) {
		return fmt.Errorf("TOTP struct is not valid: Time interval should be between %vs and %vs, but the current interval is %v", minIntervalSec, maxIntervalSec, totp.Interval.Seconds())
	}
	return nil
}
