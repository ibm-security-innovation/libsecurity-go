...
func initAListOfUsers(t *testing.T, usersList []string) string {
	cookieStr, _ := app.GenerateToken(stc.RootUserName, am.SuperUserPermission, listener, stRestful.SignKey)
...
func TestUpdatePrivilege(t *testing.T) {
	...
	cookieStr, _ := app.GenerateToken(userName, am.UserPermission, listener, stRestful.SignKey)
	cr.SetCookie(cookieStr)
